    ...
    handler: async (ctx, args) => {
        ...
        const completion = await openai.chat.completions.create({
            messages: [{
                role: 'user',
                content: `
                Summarize the following adventure of a text based RPG and return a json string with the following format so that i can know what  inventory i have,
                and also what my health points are.
                ...
                please only give me JSON. no other output.`
            }],
            model: 'gpt-3.5-turbo'
        })
        ...
        await Promise.all(
            stats.inventory.map((itemName: string | null) => {
